...
static unsigned int psk_server_callback(SSL *ssl, const char *identity, unsigned char *psk, unsigned int max_psk_len)
{
	...
	context = (mosquitto*) SSL_get_ex_data(ssl, tls_ex_index_context);
	if(!context) return 0;

	listener = (mosquitto__listener*) SSL_get_ex_data(ssl, tls_ex_index_listener);
	...
	psk_key = (char*) mosquitto__calloc(1, max_psk_len*2 + 1);
	if(!psk_key) return 0;
...
int net__socket_listen(struct mosquitto__listener *listener)
{
	...
	for(rp = ainfo; rp; rp = rp->ai_next){
		...
		listener->sock_count++;
		listener->socks = (mosq_sock_t*) mosquitto__realloc(listener->socks, sizeof(mosq_sock_t)*listener->sock_count);
		if(!listener->socks){
