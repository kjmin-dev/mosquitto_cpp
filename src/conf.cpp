...
int config__parse_args(struct mosquitto_db *db, struct mosquitto__config *config, int argc, char *argv[])
{
	...
	if(config->listener_count == 0
		...
		config->listeners = (mosquitto__listener*) mosquitto__realloc(config->listeners, sizeof(struct mosquitto__listener)*config->listener_count);
		if(!config->listeners){
			...
			if(token){
				if(!strcmp(token, "acl_file")){
					...
					while((token = strtok_r(NULL, " ", &saveptr))){
						...
						cur_bridge->addresses = (bridge_address*) mosquitto__realloc(cur_bridge->addresses, sizeof(struct bridge_address)*cur_bridge->address_count);
					...
					if(token[0]){
						...
						cur_auth_plugin_config->options = (mosquitto_opt*) mosquitto__realloc(cur_auth_plugin_config->options, cur_auth_plugin_config->option_count*sizeof(struct mosquitto_auth_opt));
					...
					conf__set_cur_security_options(config, cur_listener, &cur_security_options);
					cur_security_options->auth_plugin_configs = (mosquitto__auth_plugin_config*) mosquitto__realloc(cur_security_options->auth_plugin_configs, (cur_security_options->auth_plugin_config_count+1)*sizeof(struct mosquitto__auth_plugin_config));
					...
					if(token){
						...
						config->bridge_count++;
						config->bridges = (mosquitto__bridge*) mosquitto__realloc(config->bridges, config->bridge_count*sizeof(struct mosquitto__bridge));
						if(!config->bridges){
							log__printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
							return MOSQ_ERR_NOMEM;
						}
						...
						while((de = readdir(dh)) != NULL){
							if(strlen(de->d_name) > 5){
								if(!strcmp(&de->d_name[strlen(de->d_name)-5], ".conf")){
									...
									conf_file = (char*) mosquitto__malloc(len+1);
									if(!conf_file){
										closedir(dh);
										return MOSQ_ERR_NOMEM;
									}
					...
					if(token){
						...
						cur_bridge->topics = (mosquitto__bridge_topic*) mosquitto__realloc(cur_bridge->topics,
					...
					if(cur_topic->local_prefix){
						if(cur_topic->topic){
							...
							cur_topic->local_topic = (char*) mosquitto__malloc(len+1);
					...
					if(cur_topic->remote_prefix){
						if(cur_topic->topic){
							...
							cur_topic->remote_topic = (char*) mosquitto__malloc(len+1);
...
int config__read_file(struct mosquitto__config *config, bool reload, const char *file, struct config_recurse *cr, int level, int *lineno)
{
	...
	buflen = 1000;
	buf = (char*) mosquitto__malloc(buflen);
	if(!buf){
		log__printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
		fclose(fptr);
		return MOSQ_ERR_NOMEM;
