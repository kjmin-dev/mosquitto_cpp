...
int add__acl(struct mosquitto__security_options *security_opts, const char *user, const char *topic, int access)
{
	...
	if(!acl_user){
		acl_user = (mosquitto__acl_user*) mosquitto__malloc(sizeof(struct mosquitto__acl_user));
		if(!acl_user){
	...
	acl = (mosquitto__acl*) mosquitto__malloc(sizeof(struct mosquitto__acl));
	if(!acl){
		mosquitto__free(local_topic);
...
static int pwfile__parse(const char *file, struct mosquitto__unpwd **root)
{
	...
	while(!feof(pwfile)){
		if(fgets(buf, 256, pwfile)){
			...
			if(username){
				unpwd = (mosquitto__unpwd*) mosquitto__calloc(1, sizeof(struct mosquitto__unpwd));
				if(!unpwd){
					fclose(pwfile);
					return MOSQ_ERR_NOMEM;
				}
...
int base64__decode(char *in, unsigned char **decoded, unsigned int *decoded_len)
{
	...
	*decoded = (unsigned char*) mosquitto__calloc(strlen(in), 1);
