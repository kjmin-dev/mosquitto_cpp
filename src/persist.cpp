...
static int persist__client_msg_restore(struct mosquitto_db *db, const char *client_id, uint16_t mid, uint8_t qos, uint8_t retain, uint8_t direction, uint8_t state, uint8_t dup, uint64_t store_id)
{
	...
	cmsg = (mosquitto_client_msg*) mosquitto__malloc(sizeof(struct mosquitto_client_msg));
	if(!cmsg){
...
static int persist__client_chunk_restore(struct mosquitto_db *db, FILE *db_fptr)
{
	...
	client_id = (char*) mosquitto__malloc(slen+1);
	if(!client_id){
		fclose(db_fptr);
		log__printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
		return MOSQ_ERR_NOMEM;
...
static int persist__msg_store_chunk_restore(struct mosquitto_db *db, FILE *db_fptr)
{
	...
	load = (mosquitto_msg_store_load*) mosquitto__malloc(sizeof(struct mosquitto_msg_store_load));
	if(!load){
		fclose(db_fptr);
		log__printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
		return MOSQ_ERR_NOMEM;
	...
	slen = ntohs(i16temp);
	if(slen){
		source_id = (char*) mosquitto__malloc(slen+1);
		if(!source_id){
	...
	slen = ntohs(i16temp);
	if(slen){
		topic = (char*) mosquitto__malloc(slen+1);
		if(!topic){
			mosquitto__free(load);
			fclose(db_fptr);
