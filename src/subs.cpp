...
static struct sub__token *sub__topic_append(struct sub__token **tail, struct sub__token **topics, char *topic)
{
	...
	new_topic = (sub__token*) mosquitto__malloc(sizeof(struct sub__token));
	if(!new_topic){
		return NULL;
	}
	new_topic->next = NULL;
	...
	/* FIXME - this function has the potential to leak subhier, audit calling functions. */
{
	...
	if(!tokens){
		if(context && context->id){
			...
			leaf = (mosquitto__subleaf*) mosquitto__malloc(sizeof(struct mosquitto__subleaf));
			...
			if(i == context->sub_count){
				subs = (mosquitto__subhier**) mosquitto__realloc(context->subs, sizeof(struct mosquitto__subhier *)*(context->sub_count + 1));
				if(!subs){
...
struct mosquitto__subhier *sub__add_hier_entry(struct mosquitto__subhier *parent, struct mosquitto__subhier **sibling, const char *topic, size_t len)
{
	...
	child = (mosquitto__subhier*) mosquitto__malloc(sizeof(struct mosquitto__subhier));
	if(!child){
