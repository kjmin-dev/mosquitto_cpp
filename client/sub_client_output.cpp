...
static void json_print(const struct mosquitto_message *message, const struct tm *ti, bool escaped)
{
	...
	if(escaped){
		...
		fputs("\"payload\":", stdout);
		write_payload((const unsigned char*) message->payload, message->payloadlen, 0);
		fputs("}", stdout);
	}
}
		...
		if(cfg->format[i] == '%'){
			if(i < len-1){
				...
				switch(cfg->format[i]){
					...
					case 'x':
						write_payload((const unsigned char*) message->payload, message->payloadlen, 1);
						break;

					case 'X':
						write_payload((const unsigned char*) message->payload, message->payloadlen, 2);
						break;
				}
			}
...
void print_message(struct mosq_config *cfg, const struct mosquitto_message *message)
{
	if(cfg->format){
		...
		if(message->payloadlen){
			...
			write_payload((const unsigned char*) message->payload, message->payloadlen, false);
			if(cfg->eol){
				printf("\n");
			}
		}else{
